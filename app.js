...
function normalizeDB(){
  ...
  db.invoices.forEach(inv=>{
    ...
    if(inv.dueDate===undefined) inv.dueDate="";
    if(!inv.customerId){
...
function invoiceDisplayStatus(inv){
  ...
  if(inv.dueDate && (+inv.balance||0)>0.0005){
    const due=String(inv.dueDate).slice(0,10);
    if(due && due<todayLocalISODate()) return "overdue";
  }
...
function startInvoice(){
  ...
  if(document.getElementById("invoiceDueDate")) invoiceDueDate.value="";
...
function currentInvoiceObj(){
  ...
  return{
    ...
    dueDate:(document.getElementById("invoiceDueDate")?invoiceDueDate.value:""),
...
function editSavedInvoice(i){
  ...
  if(document.getElementById("invoiceDueDate")) invoiceDueDate.value=d.dueDate||"";
...
function renderCustomerStatement(){
  ...
  const total=invs.reduce((s,x)=>s+(+x.total||0),0);
  const paid=invs.reduce((s,x)=>s+(+x.paid||0),0);
  const balance=invs.reduce((s,x)=>s+(+x.balance||0),0);
  ...
  statementRows.innerHTML=invs.length ? invs.map(x=>`
    <tr>
      <td>${new Date(x.date).toLocaleDateString(lang==="ar"?"ar-OM":"en-GB")}</td>
      <td><button class="statement-invoice-link" onclick="openStatementInvoice('${x.number}')">${esc(x.number)}</button></td>
      <td>${money(x.total)}</td>
      <td>${money(x.paid)}</td>
      <td class="${(+x.balance||0)>0?"statement-due":"statement-paid"}">${money(x.balance)}</td>
    </tr>`).join("") : `<tr><td colspan="5" class="empty">${tr("No invoices for this customer","لا توجد فواتير لهذا العميل")}</td></tr>`;
  ...
  if(document.getElementById("statementPaymentRows")){
    statementPaymentRows.innerHTML=payments.length ? payments.slice().reverse().map(p=>{
      ...
      return `<tr class="payment-history-row">
        ...
        <td>${new Date((p.date||todayLocalISODate())+"T00:00:00").toLocaleDateString(lang==="ar"?"ar-OM":"en-GB")}</td>
...
async function renderStatementCanvases(){
  const {customer,invoices}=statementPDFData();
  const s=db.settings||{};
  const rtl=lang==="ar";
  const W=1240,H=1754,L=75,R=1165;
  const rowsPerPage=15;
  const chunks=[];
  if(!invoices.length) chunks.push([]);
  for(let i=0;i<invoices.length;i+=rowsPerPage) chunks.push(invoices.slice(i,i+rowsPerPage));
  const totalAll=invoices.reduce((n,x)=>n+(+x.total||0),0);
  const paidAll=invoices.reduce((n,x)=>n+(+x.paid||0),0);
  const balAll=invoices.reduce((n,x)=>n+(+x.balance||0),0);
  const logo=await loadImageForCanvas(companyLogo());
  const canvases=[];
  ...
  for(let p=0;p<chunks.length;p++){
    ...
    const headers=rtl?[tr("Balance","المتبقي"),tr("Paid","المدفوع"),tr("Total","الإجمالي"),tr("Invoice","الفاتورة"),tr("Date","التاريخ")]:[tr("Date","التاريخ"),tr("Invoice","الفاتورة"),tr("Total","الإجمالي"),tr("Paid","المدفوع"),tr("Balance","المتبقي")];
    ...
    let yy=y0+56;ctx.font="20px -apple-system,Arial";ctx.fillStyle=TEXT;
    rows.forEach(inv=>{
      const vals=rtl?[money(inv.balance),money(inv.paid),money(inv.total),inv.number,new Date(inv.date).toLocaleDateString("ar-OM")]:[new Date(inv.date).toLocaleDateString("en-GB"),inv.number,money(inv.total),money(inv.paid),money(inv.balance)];
      let xx=L;
      vals.forEach((v,i)=>{ctx.textAlign="center";ctx.fillText(String(v),xx+widths[i]/2,yy+37);xx+=widths[i];});
      ctx.strokeStyle="#dde3ed";ctx.beginPath();ctx.moveTo(L,yy+55);ctx.lineTo(R,yy+55);ctx.stroke(); yy+=55;
    });

    if(p===chunks.length-1){
      const ty=1390, tw=520, tx=rtl?L:R-tw;
      const totals=[[tr("Total Invoiced","إجمالي الفواتير"),money(totalAll)],[tr("Total Paid","إجمالي المدفوع"),money(paidAll)],[tr("Outstanding","المتبقي"),money(balAll)]];
      totals.forEach((r,i)=>{
        ctx.fillStyle=i===2?GOLD:(i===1?BLUE:"#fff");ctx.fillRect(tx,ty+i*58,tw,58);ctx.strokeStyle="#d8dee9";ctx.strokeRect(tx,ty+i*58,tw,58);
        ctx.fillStyle=i===1?"#fff":TEXT;ctx.font=(i===2?"700 ":"")+"22px -apple-system,Arial";
        ctx.textAlign=rtl?"right":"left";ctx.fillText(r[0],rtl?tx+tw-18:tx+18,ty+i*58+37);
        ctx.textAlign=rtl?"left":"right";ctx.fillText(r[1],rtl?tx+18:tx+tw-18,ty+i*58+37);
      });
    }
...
async function exportBackup(){
  ...
  const payload={
    app:"Invoice Pro",
    version:"2026.09.09-FINANCEPLUS",
