...
function buildCustomerStatementLedger(i){
  const invoices=customerInvoicesByIndex(i).map((inv,idx)=>({
    kind:"invoice",
    date:inv.date||"",
    ref:inv.number||"",
    ...
    credit:0,
    sortStamp:(new Date((inv.date||"1970-01-01")+"T00:00:00").getTime()||0)+idx/1000
  }));
  ...
  const payments=customerPaymentsByIndex(i).map((p,idx)=>({
    ...
    sortStamp:(new Date((p.date||"1970-01-01")+"T00:00:00").getTime()||0)+0.5+idx/1000
  }));

  const rows=invoices.concat(payments).sort((a,b)=>a.sortStamp-b.sortStamp || String(a.ref).localeCompare(String(b.ref)));
  let running=0;
  return rows.map(r=>{
...
async function renderStatementCanvases(){
  ...
  const paidAll=payments.reduce((n,x)=>n+(+x.amount||0),0);
